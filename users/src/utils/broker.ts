...
import { MSG_QUEUE_URL, EXCHANGE, USER_SERVICE, USER_QUEUE } from "../config/secrets";
...
export class MessageBroker {
  ...
  async subscribeMessage(chan: Channel): Promise<void> {
    chan.assertExchange(EXCHANGE, "direct", { durable: true });
    const q = await chan.assertQueue(USER_QUEUE, { durable: true });
    console.log(` Waiting for messages in queue: ${q.queue}`);
    chan.bindQueue(q.queue, EXCHANGE, USER_SERVICE);
    chan.consume(
      q.queue,
      (message) => {
        if (message?.content) {
